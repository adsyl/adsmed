(function(){"use strict";function e(){let e=null;const t=document.getElementById("pgtForm");if(!t)return;t.addEventListener("submit",function(t){t.preventDefault();const n={age:parseInt(document.getElementById("age").value),amh:parseFloat(document.getElementById("amh").value),afc:parseInt(document.getElementById("afc").value),fsh:parseFloat(document.getElementById("fsh").value),lh:parseFloat(document.getElementById("lh").value),e2:parseFloat(document.getElementById("e2").value),uterine:document.getElementById("uterineCondition").value,otherMedicalHistory:document.getElementById("otherMedicalHistory").value,liveBirth:parseInt(document.getElementById("liveBirthsCount").value),miscarriage:parseInt(document.getElementById("miscarriageCount").value),height:parseFloat(document.getElementById("heightCm").value),weight:parseFloat(document.getElementById("weightKg").value)},C=e=>{let n=e.age<=30?40:e.age<=35?35:e.age<=38?22:e.age<=42?11:4,s=e.amh<1?.5:e.amh<=2?.9:e.amh<5?1.5:1.6,o=e.afc<4?.35:e.afc<=8?.8:e.afc<15?1.25:1.6,i=o*.6+s*.4,t=e.fsh<8?1.2:e.fsh<=10?1:e.fsh<=12?.75:e.fsh<=15?.45:.15;(e.lh/e.fsh>2.5||e.lh<1.5)&&(t*=.8);let a=e.e2<50?1:e.e2<=80?.85:.6;return Math.min(Math.max(Math.round(n*i*t*a*1.8),1),100)},a=C(n),j=n.fsh>15?.5:n.fsh>12?.75:1,x=(.6+Math.min(n.amh,4)/4*.3)*j*(n.e2>80?.8:1),m=Math.min(Math.max(n.afc*x,Math.min(n.afc,1)),n.afc*.9),y=e=>e<=30?.9:e<=35?.85:e<=38?.8:e<=40?.7:e<=42?.55:.3,O=y(n.age);(n.lh/n.fsh>2||n.lh/n.fsh<.5)&&(conv*=.82);const h=m*O*conv*(.75+.25*(a/100)),A=e=>e<=30?.85:e<=35?.75:e<=38?.5:e<=40?.32:e<=42?.18:.08,k=A(n.age),l=h*k,M=e=>e<=30?.8:e<=35?.7:e<=38?.55:e<=40?.35:e<=42?.18:.05,S=M(n.age),i=l*S,L=i>0?.99-(1-eRate)**Math.max(l,1):0,r=n.weight/(n.height/100)**2;let b={normal:1,fibroids_polyps_managed:.92,adenomyosis_endo:.55,congenital_anomalies:.82,other_unmanaged:.4}[n.uterine],o=1;n.liveBirth>0?(o=1.1,n.miscarriage>=3?o*=.8:n.miscarriage>0&&(o*=.9)):n.miscarriage==1?o=.95:n.miscarriage==2?o=.85:n.miscarriage>=3&&(o=.65);let _=r>=28?.8:r<18.5?.85:r>=25?.95:1;const w={none:1,mild_managed:.94,immune_severe_managed:.78,unmanaged_severe:.42}[n.otherMedicalHistory],s=b*o*_*w,c=Math.min(Math.max(.68*s,.05),.72),v=Math.log(.15)/Math.log(1-c),E=Math.max(i,.1);let u=Math.ceil(v/E);i>=v*.85&&(u=1);const g=Math.min(u,6),p=Math.ceil(Math.log(.15)/Math.log(1-c)),f=Math.min(Math.max(i>0?(1-(1-c)**Math.max(i,1))*100:a*.32,2),89),F=e=>e<=30?"【最佳】卵子数量和质量最佳，胚胎染色体异常率处于很低水平。":e<=35?"【良好】卵子数量和质量较好，胚胎染色体异常率处于较低水平。":e<=38?"【一般】卵子数量和质量一般，胚胎染色体异常率相对升高。":e<=40?"【下降】卵子数量和质量显著下降，染色体非整倍体风险显著升高。":"【预警】卵子高龄效应明显，需重点关注胚胎过检率及活产率的对冲风险。",T=e=>e>=4?"【理想】卵巢储备充沛，获卵基数优势巨大，单次促排获得健康胚胎几率极大。":e>=2.5?"【一般】卵巢储备一般，获卵基数符合预期，可能需要1-2次促排。":e>=1?"【偏低】卵巢储备下滑，获卵基数相对较少，多次促排是必要选择。":"【预警】卵巢储备不足，建议采用多次促排积攒胚胎策略或考虑供卵策略。",z=(e,t)=>e==="normal"&&t>=.95?"【理想】子宫环境良好，内膜容受性经评估处于最优状态，利于胚胎定植。":e==="adenomyosis_endo"?"【严峻】内异症/腺肌症可能干扰局部微环境，建议移植前进行降调或免疫调理。":"【需关注】环境存在折损风险，建议通过控制体重、宫腔复查或内膜准备等流程优化环境。";document.getElementById("lbrDisplay").innerText=f.toFixed(1)+"%",document.getElementById("cycleDisplay").innerText=`${g} 促 ${p} 移`;const D=[{title:"种子储备/质量",status:a>=80?"最优":a>=60?"良好":a>=40?"一般":"需调理",info:`评分 ${a}，预估单次促排可获健康胚胎 ${i.toFixed(1)} 枚。`},{title:"健康囊胚预期",status:n.age<=30?"最优":n.age<=38?"良好":n.age<=42?"一般":"需调理",info:`${n.age}岁段 PGT 过检率约 ${(eRate*100).toFixed(0)}%。`},{title:"土壤/全身状况",status:s>=.88?"最优":s>=.75?"良好":s>=.6?"一般":"需调理",info:`环境系数 ${(s*100).toFixed(0)}%，单胚胎着床率约 ${(c*100).toFixed(1)}%。`}];document.getElementById("dimensionCards").innerHTML=D.map(e=>`
                ...
                <section>
                    ...
                    <div class="space-y-6 pl-3">
                        <div>
                            ...
                            <p class="text-xl font-black text-white italic">预计需 ${g} 个促排周期</p>
                        </div>
                        <div>
                            <p class="text-gray-400 text-xs mb-1">实现累计 LBR > 85%成功率</p>
                            <p class="text-xl font-black text-white italic">预计需 ${p} 次移植周期</p>
                        </div>
