(function(){"use strict";function e(){let e=null;const t=document.getElementById("pgtForm");if(!t)return;t.addEventListener("submit",function(t){t.preventDefault();const n={age:parseInt(document.getElementById("age").value),amh:parseFloat(document.getElementById("amh").value),afc:parseInt(document.getElementById("afc").value),fsh:parseFloat(document.getElementById("fsh").value),lh:parseFloat(document.getElementById("lh").value),e2:parseFloat(document.getElementById("e2").value),uterine:document.getElementById("uterineCondition").value,otherMedicalHistory:document.getElementById("otherMedicalHistory").value,liveBirth:parseInt(document.getElementById("liveBirthsCount").value),miscarriage:parseInt(document.getElementById("miscarriageCount").value),height:parseFloat(document.getElementById("heightCm").value),weight:parseFloat(document.getElementById("weightKg").value)},E=e=>{let n=e.age<=30?40:e.age<=35?35:e.age<=38?22:e.age<=42?11:4,s=e.amh<1?.5:e.amh<=2?.9:e.amh<5?1.5:1.6,o=e.afc<4?.35:e.afc<=8?.8:e.afc<15?1.25:1.6,i=o*.6+s*.4,t=e.fsh<8?1.2:e.fsh<=10?1:e.fsh<=12?.75:e.fsh<=15?.4:.15;(e.lh/e.fsh>2.5||e.lh<1.5)&&(t*=.8);let a=e.e2<50?1:e.e2<=80?.85:.6;return Math.min(Math.max(Math.round(n*i*t*a*1.8),1),100)},a=E(n),_=(.6+Math.min(n.amh,4)/4*.3)*(n.fsh>12?.8:1)*(n.e2>80?.8:1),h=Math.min(Math.max(n.afc*_,Math.min(n.afc,1)),n.afc*.9),y=e=>e<=30?.7:e<=35?.65:e<=38?.5:e<=40?.32:e<=42?.18:.08;let p=.85*.9*y(n.age);(n.lh/n.fsh>2||n.lh/n.fsh<.5)&&(p*=.85);const l=h*p*(.75+.25*(a/100)),C=e=>e<=30?.75:e<=35?.6:e<=38?.48:e<=40?.3:e<=42?.16:.05,d=C(n.age),i=l*d,T=i>0?.99-(1-d)**Math.max(l,1):0,r=n.weight/(n.height/100)**2;let j={normal:1,fibroids_polyps_managed:.92,adenomyosis_endo:.55,congenital_anomalies:.82,other_unmanaged:.4}[n.uterine],o=1;n.liveBirth>0?(o=1.1,n.miscarriage>=3?o*=.8:n.miscarriage>0&&(o*=.9)):n.miscarriage==1?o=.95:n.miscarriage==2?o=.85:n.miscarriage>=3&&(o=.65);let x=r>=28?.85:r<18.5?.9:r>=25?.95:1;const O={none:1,mild_managed:.94,immune_severe_managed:.78,unmanaged_severe:.42}[n.otherMedicalHistory],s=j*o*x*O,c=Math.min(Math.max(.68*s,.05),.72),f=Math.log(.15)/Math.log(1-c),w=Math.max(i,.1);let m=Math.ceil(f/w);i>=f*.85&&(m=1);const b=Math.min(m,6),v=Math.ceil(Math.log(1-.85)/Math.log(1-c)),g=Math.min(Math.max(i>0?(1-(1-c)**Math.max(i,1))*100:a*.32,2),89),k=e=>e<=30?"【最佳】卵子数量和质量最佳，胚胎染色体异常率处于很低水平。":e<=35?"【良好】卵子数量和质量较好，胚胎染色体异常率处于较低水平。":e<=38?"【一般】卵子数量和质量一般，胚胎染色体异常率相对升高。":e<=40?"【下降】卵子数量和质量显著下降，染色体非整倍体风险显著升高。":"【预警】卵子高龄效应明显，需重点关注胚胎过检率及活产率的对冲风险。",A=e=>e>=4?"【理想】卵巢储备充沛，获卵基数优势巨大，单次促排获得健康胚胎几率极大。":e>=2.5?"【一般】卵巢储备一般，获卵基数符合预期，可能需要1-2次促排。":e>=1?"【偏低】卵巢储备下滑，获卵基数相对较少，需要评估和考虑多次促排。":"【预警】卵巢储备不足，建议采用多次促排积攒胚胎策略或考虑供卵策略。",S=(e,t)=>e==="normal"&&t>=.95?"【理想】子宫环境良好，内膜容受性经评估处于最优状态，利于胚胎定植。":e==="adenomyosis_endo"?"【严峻】内异症/腺肌症可能干扰局部微环境，建议移植前进行降调或免疫调理。":"【需关注】环境存在折损风险，建议通过宫腔复查或内膜准备流程优化环境。";document.getElementById("lbrDisplay").innerText=g.toFixed(1)+"%",document.getElementById("cycleDisplay").innerText=`${b} 促 ${v} 移`;const M=[{title:"种子储备/质量",status:a>=80?"最优":a>=60?"良好":a>=40?"一般":"需调理",info:`评分 ${a}，预估单次促排可获健康胚胎 ${i.toFixed(1)} 枚。`},{title:"健康囊胚预期",status:n.age<=30?"最优":n.age<=38?"良好":n.age<=42?"一般":"需调理",info:`${n.age}岁段 PGT 过检率约 ${(d*100).toFixed(0)}%。`},{title:"土壤/全身状况",status:s>=.88?"最优":s>=.75?"良好":s>=.6?"一般":"需调理",info:`环境系数 ${(s*100).toFixed(0)}%，单胚胎着床率约 ${(c*100).toFixed(1)}%。`}];document.getElementById("dimensionCards").innerHTML=M.map(e=>`
                ...
                <section class="border-b border-gray-700 pb-10">
                    ...
                    <div class="space-y-8 text-gray-300 text-sm leading-relaxed">
                        ...
                        <div>
                            ...
                            <p class="pl-3 border-l border-gray-700">
                                ${S(n.uterine,s)} 
                                ${n.liveBirth>0?"既往活产史是子宫环境的有力证明。":n.miscarriage>=2?"多次流产病史强烈建议行胚胎染色体筛查。":"无显著不利病史，建议按常规流程执行。"}
                            </p>
                        </div>
                    </div>
                </section>
                <section class="border-b border-gray-700 pb-10">
                    ...
                    <div class="space-y-4 pl-3">
                        ...
                        <p class="text-gray-300 text-sm"><span class="text-gray-500 font-bold mr-2">【促排策略】</span>${m>1?"鉴于单周期产出预估低于积攒目标，建议采用积攒周期策略。":"当前取卵预期可覆盖移植消耗，首选单周期方案。"}</p>
                    </div>
                </section>
                <section>
